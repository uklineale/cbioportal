...
public class IndexPageController {
    ...
    @GetMapping(value = {"/login", "/login.html"}, produces = MediaType.APPLICATION_JSON_VALUE)
    public String showNewLoginPage(HttpServletRequest request, Authentication authentication, Model model) {
        ...
        model.addAttribute("login_error", Boolean.parseBoolean(request.getParameter("login_error")));
        model.addAttribute("show_saml", frontendPropertiesService.getFrontendProperty(FrontendProperty.authenticationMethod).equals("saml"));
        model.addAttribute("show_google", Arrays.asList(authenticate).contains("optional_oauth2") || Arrays.asList(authenticate).contains("oauth2"));
        model.addAttribute("show_microsoft", Arrays.asList(authenticate).contains("optional_oauth2") || Arrays.asList(authenticate).contains("oauth2"));
